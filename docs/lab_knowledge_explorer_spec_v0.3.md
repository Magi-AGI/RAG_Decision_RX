...
> Status: working draft under multi-reviewer iteration (Claude drafts; Codex + Gemini review). This document is the canonical baseline for the implementation plan. Repo placement is provisional (see §16).
...
Strong structure to exploit: named entities throughout; folder + filename encode `project / experiment / date`; provenance (doc numbers, dated filenames, attendees) supports citations and timelines. Filter `.DS_Store`/`Thumbs.db`/`.psd`/`.meg`; dedupe `.doc↔.docx`.
...
**Layer 2 — RAG service** (Python, isolated container, new repo `magi-rag-service`): async ingestion worker → convert → LlamaParse → parent-child chunk → entity extract → embed → Qdrant + BM25. Query path per §7. Reads/writes cards via the Decko MCP/REST API (JWT/role auth) — "just another MCP client."
...
- Re-sync **coarse permission tags** (§7) on permission-change.
...
**Confidence is two-axis** — *directness of evidence* × *cross-source corroboration* — so a single authoritative method report can be **high**, not auto-medium. Rendered as `high | medium | low` with both axes recorded.
...
| Retriever | §7 pipeline; configurable `candidate_N`/`authorized_top_k`/cap |
| Synthesizer | configurable Claude / GPT-4o (§13 governance); fuzzy quote validation; enriched schema |
...
- **Smoke set (Week 1):** 10–15 pilot queries incl. **negative/no-answer** cases.
...
- **Embedding benchmark (Phase 1):** BGE-large vs **biomedical/scientific embedding candidates** (e.g. PubMedBERT, SPECTER2, others) on the smoke set before locking acceptance targets.
...
| Dual-write inconsistency | Single bucket-of-record; canonical ownership (§5) |
...
| 12-week scope slip | Parallel tracks (§16); pilot corpus first; Streamlit not custom UI |

## 16. Implementation shape & repository boundary
**Parallel tracks (detailed week-by-week in the implementation plan):**
- **Track A — Python pipeline/algorithmic:** mock folder layout, chunker, entity extractor, Qdrant/BM25 retrieval + synthesis, Streamlit chunk-review, eval datasets.
...
- **Track D — refinement & deployment:** biomedical-embedding benchmark, legacy-conversion scale testing, runbooks, deployment.
...
## 18. Open questions
1. Deck ingestion in MVP vs deferred entirely to the vision track, given decks are mostly image-borne?
